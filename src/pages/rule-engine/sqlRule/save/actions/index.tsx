...
const Action: React.FC<Props> = props => {
      ...
      return (
        <Col span={6} style={{ paddingBottom: 10 }}>
          <Select
            placeholder="选择属性值"
            defaultValue={
              actionData.configuration.message?.properties[propertiesData.id][properties.id] ||
              undefined
            }
  ...
  const renderProperties = () => {
    ...
    if (propertiesData.valueType.type === 'enum') {
      ...
      return (
        <Col span={24}>
          {arrayData[0] &&
            arrayData.map((value: string, index: number) => (
      ...
      return propertiesData.valueType.properties?.map((item: any, index: number) => (
          ...
          <div>
            <Col span={6} style={{ paddingBottom: 10 }}>
              <Select
                ...
                defaultValue={
                  actionData.configuration.message?.properties
                    ? Object.keys(actionData.configuration.message?.properties)[0]
                    : undefined
                }
              ...
              >
                {JSON.parse(deviceData.metadata).properties?.map((item: any) => (
                  <Select.Option
                    key={item.id}
                    data={item}
                  >{`${item.name}（${item.id}）`}</Select.Option>
                ))}
              </Select>
            </Col>
      ...
      case 'INVOKE_FUNCTION':
        return (
          <div>
            ...
            {functionData.id &&
              functionData.inputs.map((item: any, index: number) => {
                return (
                  <Col span={24} style={{ marginLeft: -8 }}>
        ...
        return (
          <div>
            <Col span={4}>
              <Select
                placeholder="选择通知类型"
                value={actionData.configuration?.notifyType || undefined}
              ...
              >
                {notifyTypeConfig.length > 0 &&
                  notifyTypeConfig.map((item: any) => (
            ...
            <Col span={6}>
              <Select
                placeholder="选择通知配置"
                value={actionData.configuration?.notifierId || undefined}
                onChange={(value: string, event: any) => {
            ...
            <Col span={6}>
              <Select
                placeholder="选择通知模板"
                value={actionData.configuration?.templateId || undefined}
                onChange={(value: string) => {
  ...
  useEffect(() => {
    setActionType('device-message-sender');
    actionData.executor = 'device-message-sender';
    setActionData({ ...actionData });
    submitData();
  }, []);
    ...
    <div style={{ paddingBottom: 5 }}>
      <Card size="small" bordered={false} style={{ backgroundColor: '#F5F5F6' }}>
        <Row>
          ...
          <Popconfirm title="确认删除此执行动作？" onConfirm={() => props.remove(props.position)}>
